...
export function ModalLogin({ onSuccess, onSwitchToRegister }) {
  const {
    register,
    handleSubmit,
    formState: { errors }
  ...
  return (
    <form className="grid grid-cols-4" onSubmit={handleSubmit(onSubmit)}>
      <div className="bg-white p-8 rounded-md shadow-md col-span-full xl:col-span-4">
        ...
        <GoogleLoginButton onError={setServerError} onSuccess={onSuccess} />
        <div className="mt-4">
          <span className="text-sm text-gray-900 font-bold">Rather use email? </span>
          <span
